...
import { orderAll } from '@/api/product';
...
export default {
	...
	onLoad(options) {
		 ...
		 * 替换onLoad下代码即可
		 */
		this.tabCurrentIndex = parseInt(options.state, 10) || 0;
	},
	// 下拉刷新
	onPullDownRefresh() {
		this.page = 1;
	...
	methods: {
		...
		async getOrderList(type) {
			...
			this.req.page_size = this.pageSize
			// String(navItem.state)
			this.req.filter[0].value = String(this.tabCurrentIndex)
			await this.$http
				.post(`${orderAll}`, this.req)
				.then(async r => {
					if (type === 'refresh') {
