...
export default class UniSockt{
	constructor(config){
		this.curSocket = null 				// 当前socket对象
		this.currSocket = null; 			// 心跳包时间索引
		this.timer = null; 					// 断线重连时间索引
		this.reConnectTimer = null; 		// 是否登录
		this.isLogin = false; 				// 当前网络状态
		this.netWork = true; 				// 是否是主动退出
		this.socketMsgQueue = []; 			// 消息队列
		this.copyIsReconnection = !!config.isReconnection; // 是否重连
		...
		success: (res1) => {
			if (res1.networkType === "none") {
				...
				this.currSocket = uni.connectSocket({
					...
					header: {},
					fail: (err) => {
						this.config.debug &&console.log("%c [socket] %c 接口API调用失败:","color:red;","color:#000;",err);
						typeof fail === "function" && fail(err, this);
					},
					success: (res2) => {
				...
				this.currSocket.onOpen((header) => {
					this.config.debug && console.log("%c [socket] %c socket打开成功","color:blue;","color:#000;",header);
					this.isLogin = true;
					// 判断是否需要发送心跳包
					if (this.config.isSendHeart) {
						this.config.debug && console.log("%c [socket] %c 检查到需要发送心跳包:","color:#0f0;","color:#000;",this.config.timeout);
						this._clearHeart();
						this._startHeart();
					}
					// 发送消息队列消息
					for (let i = 0; i < this.socketMsgQueue.length; i++) {
						this.config.debug && console.log("%c [socket] %c 正在发送消息队列消息:","color:blue;","color:#000;",i,this.socketMsgQueue[i]);
				...
				this.currSocket.onClose((res) => {
					...
					this.config.debug && console.log("%c [socket] %c 监听到socket被关闭了:","color:red;","color:#000;",JSON.stringify(res));
					// 停止心跳检查
					if (this.config.isSendHeart) this._clearHeart();
